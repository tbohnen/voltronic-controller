...
const mqtt = require('./bridge-mqtt');
...
const getStatus = (cached = true) => {
	return new Promise((resolve, reject) => { 
		...
		exec("sudo sh -c './inverter_poller -1'", (err, stdout, stderr) => {
		  if (err) {
			console.error(err);
			reject(err);
		    // node couldn't execute the command
		  ...
		  const date = new Date();
		  status = { type: "axpert", time: date.toISOString(), ...JSON.parse(stdout) }
...
const publishStatus = async () => {
	...
	console.log('status', status)
	mqtt.publish("/solar", JSON.stringify(status));
}

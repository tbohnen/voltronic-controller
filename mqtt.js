...
const publishStatus = async () => {
	...
	for (const key of Object.keys(status)){
		...
		console.log(`publishing (homeassistant/sensor/voltronic_${key}) ${key}:${value}`);
		client.publish(`homeassistant/sensor/voltronic_${key}`, value.toString(), [], (e) => console.log(e));
	}
}

const registerTopic = (key, unitOfMeasurement, icon) => {
	const mqttDeviceName = "voltronic"
	const topic = "homeassistant"
	
	const message = { 
		"name": `${mqttDeviceName}_${key}`,
		"unit_of_measurement": unitOfMeasurement,
		"state_topic": `${topic}/sensor/${mqttDeviceName}_${key}`,
		"icon": `mdi:${icon}`


	}
	console.log(`registering: ${topic}/sensor/${mqttDeviceName}_${key}/config: ${JSON.stringify(message)}`);

	client.publish(`${topic}/sensor/${mqttDeviceName}_${key}/config`, JSON.stringify(message), [], (e) => console.log(e));
}
...
const init = () => {
	...
	client.on('connect', function (e) {
		...
		registerTopics()
		  ...
		  client.subscribe(options.rawTopic, function (err) {
			      if (err) { console.log('could not subscribe', err); }
			  else { 
				console.log('subscribed', options.rawTopic);
			  }
			    })
	})
	...
	client.on('message', function (topic, message) {
		...
		switch (topic) {
			case options.rawTopic: {
				console.log('executing raw', message.toString());
				executeRaw(message.toString());
				break;
			}
			case options.commandsTopic: {
				console.log('executing command', message.toString());
				break;
			}
			default: {
				break;
